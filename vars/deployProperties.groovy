            ...
            [
                    ...
                    parameters(
                            [
                                    ...
                                    [$class: 'CascadeChoiceParameter', choiceType: 'PT_SINGLE_SELECT', filterLength: 1, filterable: false, name: "${config.parameterName}", randomName: 'choice-parameter-8744093374048404', referencedParameters: '', script: [$class: 'GroovyScript', fallbackScript: [classpath: [], sandbox: false, script: 'return ["latest"]'], script: [classpath: [], sandbox: false, script: '''def cmd = "/usr/local/bin/aws ecr list-images --repository-name cds-backend --filter tagStatus=TAGGED --region us-east-1 --output text"
